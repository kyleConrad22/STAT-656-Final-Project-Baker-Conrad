...
Expanding factors into series of binary variables for use in rstan. Note that the intercept term will denote the response when site_code = "WTM", treatment_code = "C1", location_within_plot = "P", subplot = 1, and region = "core".
```{r}
X <- data.frame(
  subplot = X$subplot - 1,
  west = as.integer(X$region == "west"),
  east = as.integer(X$region == "east"),
  interplant = as.integer(X$location_within_plot == "IP"),
  nit = as.integer(X$treatment_code == "N"),
  pho = as.integer(X$treatment_code == "P"),
  nit_pho = as.integer(X$treatment_code == "NP"),
  LDP = as.integer(X$treatment_code == "LDP"),
  UMP = as.integer(X$treatment_code == "LDP"),
  SRR = as.integer(X$treatment_code == "LDP"),
  MCN = as.integer(X$treatment_code == "LDP"),
  MCS = as.integer(X$treatment_code == "LDP"),
  SME = as.integer(X$treatment_code == "LDP"),
  PWP = as.integer(X$treatment_code == "LDP"),
  MVP = as.integer(X$treatment_code == "LDP"),
  SMW = as.integer(X$treatment_code == "LDP"),
  DBG = as.integer(X$treatment_code == "LDP"),
  SNE = as.integer(X$treatment_code == "LDP"),
  SNW = as.integer(X$treatment_code == "LDP"),
  EMW = as.integer(X$treatment_code == "LDP"),
  EME = as.integer(X$treatment_code == "LDP"), 
  year = X$year
  )
```

The stan code for a normal hierarchical model:
```{r}
reg_code = "
data {
